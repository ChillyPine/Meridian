...
open class DropdownFeature(
    ...
    override fun render(
        ...
        guiGraphics.text(font, label, buttonX + INNER_PAD_X, labelY, TEXT_COLOR, false)

        val arrow = if (expanded) "▲" else "▼"
        val arrowX = buttonX + buttonWidth - ARROW_ZONE + (ARROW_ZONE - font.width(arrow)) / 2
        guiGraphics.text(font, arrow, arrowX, labelY, TEXT_COLOR, false)
    ...
    override fun renderOverlay(guiGraphics: GuiGraphicsExtractor, font: Font, mouseX: Int, mouseY: Int) {
        if (!expanded) return
        ...
        options.forEachIndexed { i, option ->
            ...
            guiGraphics.text(font, option, dropdownX + INNER_PAD_X, labelY, TEXT_COLOR, false)
        }
    }
