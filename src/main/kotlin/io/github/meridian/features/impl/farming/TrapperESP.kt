        ...
        WorldRenderEvents.AFTER_ENTITIES.register { ctx ->
            ...
            for (ent in level.entitiesForRendering()) {
                ...
                if (name.contains("Trackable")) {
                    ESP.drawBox(ctx, ent, w = 1.0, h = 1.0, wz = 0.6, yOffset = -2.2, argb = 0xFFFFFFFF.toInt())
                    ESP.drawTracer(ctx, p.x, p.y - 1.0, p.z, 0xFFFFFFFF.toInt())
                } else if (name.contains("Untrackable")) {
                    ESP.drawBox(ctx, ent, w = 1.0, h = 1.0, wz = 0.6, yOffset = -2.2, argb = 0xFF00FF00.toInt())
                    ESP.drawTracer(ctx, p.x, p.y - 1.0, p.z, 0xFF00FF00.toInt())
                } else if (name.contains("Undetected")) {
                    ESP.drawBox(ctx, ent, w = 1.0, h = 1.0, wz = 0.6, yOffset = -2.2, argb = 0xFF0051FF.toInt())
                    ESP.drawTracer(ctx, p.x, p.y - 1.0, p.z, 0xFF0051FF.toInt())
                } else if (name.contains("Endangered")) {
                    ESP.drawBox(ctx, ent, w = 1.0, h = 1.0, wz = 0.6, yOffset = -2.2, argb = 0xFFD400FF.toInt())
                    ESP.drawTracer(ctx, p.x, p.y - 1.0, p.z, 0xFFD400FF.toInt())
                } else if (name.contains("Elusive")) {
                    ESP.drawBox(ctx, ent, w = 1.0, h = 1.0, wz = 0.6, yOffset = -2.2, argb = 0xFFFFF200.toInt())
